   ...
   if(isThereAnOpenTrade == false){
      ...
      if(disableTradingExceedingLoses == false){
         ...
         if( conditionForBuying == true   &&   areWaitCandlesCondtionsSatisfied == true){
            ...
            currentOrderTicket = OrderSend(Symbol(), OP_BUY, volume, Bid, 10, orderStopLoss, orderTakeProfit, "by DualBar-SUP-INF", magicNumber, clrTurquoise);
            if(OrderSelect(currentOrderTicket, SELECT_BY_TICKET) == true){
         ...
         // OPEN SELL
            ...
            currentOrderTicket = OrderSend(Symbol(), OP_SELL, volume, Bid, 10, orderStopLoss, orderTakeProfit, "by DualBar-SUP-INF", magicNumber, clrCrimson);      
            if(OrderSelect(currentOrderTicket, SELECT_BY_TICKET) == true){
               isThereAnOpenTrade = true;
               hasDrawnArrRect = false;
               if(doDrawOnOpen)
